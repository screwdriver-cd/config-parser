...
const SCHEMA_MERGE_CONFIG = require('screwdriver-data-schema').config.base.mergeConfig;
...
async function structuralValidation(doc, premergeValidation) {
    ...
    if (premergeValidation) {
        data = await SCHEMA_MERGE_CONFIG.validateAsync(doc, {
            abortEarly: false
        });
    } else {
        data = await SCHEMA_CONFIG.validateAsync(doc, {
            abortEarly: false
        });
