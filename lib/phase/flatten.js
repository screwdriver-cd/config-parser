...
function flattenStageSetupAndTeardownJobs(doc) {
    ...
    if (stages && !Hoek.deepEqual(stages, {})) {
        Object.keys(stages).forEach(stageName => {
            const { setup, teardown, jobs: stageJobs } = stages[stageName];

            stageJobs.forEach(job => {
                if (newJobs[job]) {
                    newJobs[job].stage = { name: stageName };
                    // Handle startFrom
                    if (newJobs[job].requires && newJobs[job].requires.includes(`~stage@${stageName}:setup`)) {
                        newJobs[job].stage.startFrom = true;
                        stages[stageName].startFrom = job;
                        delete newJobs[job].requires;
                    }
                }
            });

            if (!Hoek.deepEqual(setup, {})) {
                newJobs[`${STAGE_PREFIX}${stageName}:setup`] = setup;
                newJobs[`${STAGE_PREFIX}${stageName}:setup`].stage = { name: stageName };
            }
            delete stages[stageName].setup;
            if (!Hoek.deepEqual(teardown, {})) {
                newJobs[`${STAGE_PREFIX}${stageName}:teardown`] = teardown;
                newJobs[`${STAGE_PREFIX}${stageName}:teardown`].stage = { name: stageName };
            }
            delete stages[stageName].teardown;
        });
    }

    return { jobsWithStages: newJobs, newStages: stages };
}
...
module.exports = (parsedDoc, templateFactory) => {
    ...
    const { jobsWithStages, newStages } = flattenStageSetupAndTeardownJobs(parsedDoc);

    doc.jobs = jobsWithStages;

    if (newStages) {
        doc.stages = newStages;
    }
