...
const SCHEMA_STEP_RETRY = require('screwdriver-data-schema').config.job.stepRetry;
...
/**
 * Converts the simplified step retry into the most-verbose format
 *
 * This is because the user can provide either:
 *  - { "retry": true }
 *  - { "retry": { "condition": "somelogic", "max": "value", "interval": "value", "environment": { "GIT_SHALLOW_CLONE":  false } }}
 *
 * The launcher expects it in a consistent format:
 *  - { "retry": { "condition": "somelogic", "max": "value", "interval": "value", "environment": { "GIT_SHALLOW_CLONE":  false } }}
 * @method convertStepRetry
 * @param  {Object}       retry   Object with all the jobs
 * @return {Object}              New object with jobs after up-converting
 */
function convertStepRetry(retry) {
    let retryObj;

    switch (typeof retry) {
        case 'object':
            retryObj = Joi.attempt(retry, SCHEMA_STEP_RETRY);
            break;

        case 'string':
        case 'boolean':
            retryObj = Joi.attempt({}, SCHEMA_STEP_RETRY);
            break;
...
function convertSteps(jobs) {
    ...
    Object.keys(newJobs).forEach(jobName => {
        ...
        newJobs[jobName].commands = steps.map((step, index) => {
            ...
            const parsedStep = { name, command };

            if (locked) {
                parsedStep.locked = locked;
            }

            // Handle retry string vs object
            if (retry) {
                parsedStep.retry = convertStepRetry(retry);
            }
