        ...
        Object.keys(stages).forEach(stageName => {
            ...
            stageJobs.forEach(jobName => {
                if (newJobs[jobName]) {
                    ...
                    // Handle startFrom
                    if (newJobs[jobName].requires && newJobs[jobName].requires.includes(`~stage@${stageName}:setup`)) {
                        newJobs[jobName].stage.startFrom = true;
                        stages[stageName].startFrom = jobName;

                        if (typeof newJobs[jobName].requires === 'string') {
                            delete newJobs[jobName].requires;
                        } else {
                            newJobs[jobName].requires = newJobs[jobName].requires.filter(
                                req => req !== `~stage@${stageName}:setup`
                            );
                        }
                    }
                }
