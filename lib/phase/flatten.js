...
const STAGE_SETUP_JOB_PREFIX = 'sd-stage-setup-';
const STAGE_TEARDOWN_JOB_PREFIX = 'sd-stage-teardown-';
...
function flattenStageSetupAndTeardownJobs(doc) {
    const { jobs, stages } = doc;

    if (stages && !Hoek.deepEqual(stages, {})) {
        Object.keys(stages).forEach(stageName => {
            const setup = stages[stageName].setup;
            const teardown = stages[stageName].teardown;

            if (!Hoek.deepEqual(setup, {})) {
                jobs[`${STAGE_SETUP_JOB_PREFIX}${stageName}`] = setup;
            }
            delete stages[stageName].setup;
            if (!Hoek.deepEqual(teardown, {})) {
                jobs[`${STAGE_TEARDOWN_JOB_PREFIX}${stageName}`] = teardown;
            }
...
module.exports = (parsedDoc, templateFactory) => {
    ...
    flattenStageSetupAndTeardownJobs(parsedDoc);
