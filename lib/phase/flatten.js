...
function merge(newJob, oldJob, fromTemplate) {
    ...
    if (fromTemplate && oldJob.order) {
        ...
        if (!orderContainsLockedSteps) {
            ...
            const message = `Order must contain template ${oldJob.template} locked steps: ${lockedStepNames}`;
...
async function mergeTemplateIntoJob({
    ...
    return jobNames.map((jobName, i) => {
        ...
        if (!template) {
            const message = `Template ${jobConfig.template} does not exist`;
...
async function flattenPhase(parsedDoc, templateFactory) {
    ...
    if (parsedDoc.cache) {
        ...
        Object.keys(doc.jobs).forEach(jobName => {
            if (doc.jobs[jobName].cache !== undefined) {
                const message = 'Cache disable/enable is set without the cache setting itself.';
    ...
    if (errors) {
        // checkAdditionalRules returns an array of plain message strings (not Error
        // objects), so wrap it the same way the other sites here do to keep
        // toString() producing "Error: ..." as it always has.
