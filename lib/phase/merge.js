...
async function fetchPipelineTemplate(
    ...
    return pipelineTemplateVersionFactory.getTemplate(
        pipelineTemplateName,
        pipelineTemplateFactory,
        pipelineTemplateTagFactory
    );
    ...
    if (parsedDoc.jobs) {
        ...
        Object.keys(parsedDoc.jobs).forEach(jobName => {
            ...
            if (pipelineTemplateJobNames.includes(jobName)) {
                ...
                if (hasExtraField) {
                    const message = `Job "${jobName}" has unsupported fields in user yaml. Can only set ${ALLOWED_JOB_FIELDS_WITH_PIPELINE_TEMPLATE}.`;
...
module.exports = async (
    ...
    if (!pipelineTemplate) {
        const message = `Pipeline template ${pipelineTemplateName} does not exist`;
