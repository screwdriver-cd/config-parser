...
async function generateWorkflowGraph(doc, triggerFactory, pipelineId) {
    ...
    return WorkflowParser.getWorkflow({ pipelineConfig: cloneDoc, triggerFactory, pipelineId });
}
...
module.exports = async ({
    ...
    // Workflow graph
    const { pipelineWorkflow, stageWorkflows } = await generateWorkflowGraph(doc, triggerFactory, pipelineId);

    doc.workflowGraph = pipelineWorkflow;

    if (stageWorkflows) {
        Object.keys(stageWorkflows).forEach(stageName => {
            doc.stages[stageName].workflowGraph = stageWorkflows[stageName];
        });
    }
