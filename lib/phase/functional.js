...
const SDEnv = ['SD_TEMPLATE_FULLNAME', 'SD_TEMPLATE_NAME', 'SD_TEMPLATE_NAMESPACE', 'SD_TEMPLATE_VERSION'];
 ...
 */
async function generateWorkflowGraph(doc, triggerFactory, pipelineId, pipelineOnly) {
    // In case doc got changed during workflowGraph parsing
    const cloneDoc = clone(doc);

    return WorkflowParser.getWorkflow({ pipelineConfig: cloneDoc, triggerFactory, pipelineId, pipelineOnly });
}
...
module.exports = async ({
    ...
    // Workflow graph
    const workflowGraphs = await generateWorkflowGraph(doc, triggerFactory, pipelineId, pipelineOnly);

    if (pipelineOnly) {
        doc.workflowGraph = workflowGraphs;
    } else {
        doc.workflowGraph = workflowGraphs.workflow;

        if (workflowGraphs.stageWorkflows) {
            Object.keys(workflowGraphs.stageWorkflows).forEach(stage => {
                doc.stages[stage].workflowGraph = workflowGraphs.stageWorkflows[stage];
            });
        }
    }
