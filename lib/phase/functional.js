...
function generateWorkflowGraph(doc) {
    ...
    if (doc.workflow) {
        // Remove main job since workflow-parser assumes that
        cloneDoc.workflow.shift();

        return WorkflowParser.getWorkflow({
            config: cloneDoc,
            useLegacy: true
        });
    }

    return WorkflowParser.getWorkflow({ config: cloneDoc });
}
 ...
 *  - Nodes contains all defined jobs (no extra or missing)
...
function validateWorkflowGraph(doc) {
    // Validate that workflowGraph.nodes contains all jobs in `jobs`
    const sortedJobList = JSON.stringify(clone(Object.keys(doc.jobs)).sort());
    const nodesNames = clone(doc.workflowGraph.nodes).map(node => node.name);
    const filteredNodes = nodesNames.filter(name => name !== '~commit' && name !== '~pr');
    const sortedNodes = JSON.stringify(filteredNodes.sort());

    if (sortedNodes !== sortedJobList) {
        return ['WorkflowGraph: must contain all the jobs listed in "jobs" in nodes'];
    }

    // TODO: check if there is cycle in the graph
